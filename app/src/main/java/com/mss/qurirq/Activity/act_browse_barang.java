...
public class act_browse_barang extends AppCompatActivity {
    ...
    ArrayList<String> tmp_outlet = new ArrayList<>();
    ...
    public void tampilBarang(String filter, final String id_outlet){
            ...
            public void onResponse(Call<BaseResponse<Barang>> call, Response<BaseResponse<Barang>> response) {
                if (response.isSuccessful()) {
                    ...
                    for (int i = 0; i < response.body().getData().size(); i++) {
                        System.out.println(response.body().getData().get(i).getId().toString());
                        kd_brg.add(response.body().getData().get(i).getId().toString());
                        tmp_outlet.add(response.body().getData().get(i).getIdOutlet().toString());
                        nm_brg.add(response.body().getData().get(i).getNmBrg());
                        kat_brg.add(response.body().getData().get(i).getNmKategoriBarang());
